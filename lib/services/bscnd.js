...
var bitcore = require('bscncore-lib');
var zmq = require('zmq');
var async = require('async');
var LRU = require('lru-cache');
var BscnRPC = require('bscnd-rpc');
...
/**
 * Provides a friendly event driven API to bscnd in Node.js. Manages starting and
 * stopping bscnd as a child process for application support, as well as connecting
 * to multiple bscnd processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function Bscn(options) {
  if (!(this instanceof Bscn)) {
    return new Bscn(options);
  }
...
util.inherits(Bscn, Service);

Bscn.dependencies = [];

Bscn.DEFAULT_MAX_TXIDS = 1000;
Bscn.DEFAULT_MAX_HISTORY = 50;
Bscn.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Bscn.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Bscn.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Bscn.DEFAULT_SPAWN_RESTART_TIME = 5000;
Bscn.DEFAULT_SPAWN_STOP_TIME = 10000;
Bscn.DEFAULT_TRY_ALL_INTERVAL = 1000;
Bscn.DEFAULT_REINDEX_INTERVAL = 10000;
Bscn.DEFAULT_START_RETRY_INTERVAL = 5000;
Bscn.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Bscn.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Bscn.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
Bscn.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Bscn.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Bscn.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || Bscn.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Bscn.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Bscn.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Bscn.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Bscn.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Bscn.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Bscn.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Bscn.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Bscn.prototype._initCaches = function() {
...
Bscn.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Bscn.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
Bscn.prototype.getPublishEvents = function() {
  return [
    {
...
Bscn.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'bscnd/' + name, 'total:', this.subscriptions[name].length);
};

Bscn.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'bscnd/' + name, 'total:', this.subscriptions[name].length);
};


Bscn.prototype.subscribeBalance = function(emitter, addresses) {
    var self = this;
...
Bscn.prototype.unsubscribeBalance = function(emitter, addresses) {
...
Bscn.prototype.unsubscribeBalanceAll = function(emitter) {
...
Bscn.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'bscnd/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Bscn.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'bscnd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Bscn.prototype.unsubscribeAddressAll = function(emitter) {
...
Bscn.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Bscn.DEFAULT_CONFIG_SETTINGS;
...
Bscn.prototype._parseBitcoinConf = function(configPath) {
  var options = {};
...
Bscn.prototype._expandRelativeDatadir = function() {
...
Bscn.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in bscnd config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in bscnd config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in bscnd config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './bscn.conf');

  log.info('Using bscn config file:', configPath);
...
Bscn.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of bscncore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of bscncore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of bscncore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to bscnd from bitcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from bscnd. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from bscnd. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that bscnd is undergoing a reindex. ' +
             ...
             'of bscncore-node services will start.');
    node._reindex = true;
  }
};

Bscn.prototype._resetCaches = function() {
...
Bscn.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Bscn.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Bscn.prototype._initChain = function(callback) {
  var self = this;
...
Bscn.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 22002
  };
  if (this.node.network === bitcore.Networks.testnet) {
    networkOptions.rpcport = 22302;
  }
  return networkOptions;
};

Bscn.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/bscn.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/bscn.conf';
    }
  }
  return networkPath;
};

Bscn.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === bitcore.Networks.testnet) {
...
Bscn.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('bscnd/hashblock', message.toString('hex'));
    }
  }

};

Bscn.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Bscn.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Bscn Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Bscn.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Bscn.prototype._notifyBalanceSubscribers = function(txid, transaction) {
...
Bscn.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
...
Bscn.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  var addresses = this._getAddressesFromTransaction(transaction);
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('bscnd/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Bscn.prototype._zmqTransactionHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('bscnd/rawtransaction', message.toString('hex'));
    }
...
Bscn.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Bscn.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Bscn.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Bscn.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
Bscn.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('Bscn Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || Bscn.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

Bscn.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Bscn.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/bscnd.pid';
...
Bscn.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    log.info('Starting bscn process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Bscn process unexpectedly exited with code:', code);
        log.warn('Restarting bscn child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Bscn process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn bscnd.'));
      }
...
Bscn.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to bscnd.'));
    }
 ...
 */
Bscn.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Bscn configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
Bscn.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Bscn.prototype.syncPercentage = function(callback) {
  var self = this;
...
Bscn.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, bscncore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
Bscn.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Bscn.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
...
Bscn.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Bscn.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Bscn.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in bscncore is most recent to less recent
 ...
 * @param {Function} callback
 */
Bscn.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
Bscn.prototype._getConfirmationsDetail = function(transaction) {
...
Bscn.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Bscn.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Bscn.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A bscncoin transaction id
 * @param {Function} callback
 */
Bscn.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Bscn.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
Bscn.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
Bscn.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Bscn.prototype.getAddressesMempoolBalance = function(addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Bscn.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
Bscn.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Bscn.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
Bscn.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
Bscn.prototype.getJsonBlock = function(blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Bscn.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
 ...
 * @param {Function} callback
 */
Bscn.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
 ...
 * @param {Function} callback
 */
Bscn.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Bscn.prototype.estimateFee = function(blocks, callback) {
  var self = this;
...
Bscn.prototype.estimateSmartFee = function (blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Bscn.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Bscn.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Bscn.prototype.getJsonRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a Bscncore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
Bscn.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Bscn.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Bscn.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Bscn.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Bscn.prototype.getInfo = function(callback) {
...
Bscn.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Bscn.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('bscnd spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('bscnd process did not exit'));
      }
 ...
 * @param {Function} callback
 */
Bscn.prototype.listUnspent = function(minConf, maxConf, addresses, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Bscn.prototype.getNewAddress = function(callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Bscn.prototype.callContract = function(address, hash, options, callback) {
    var self = this,
 ...
 * @param {Function} callback
 */
Bscn.prototype.getAccountInfo = function(address, callback) {
 ...
 * @param {Function} callback
 */
Bscn.prototype.getSubsidy = function(height, callback) {
 ...
 * @param {Function} callback
 */
Bscn.prototype.getTransactionReceipt = function(trxHash, callback) {
 ...
 * @param {Function} callback
 */
Bscn.prototype.getDgpInfo = function(callback) {
 ...
 * @return {*}
 */
Bscn.prototype.getMiningInfo = function(callback) {
    var self = this,
 ...
 * @return {*}
 */
Bscn.prototype.getStakingInfo = function(callback) {
    var self = this,
