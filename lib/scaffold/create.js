...
var BASE_PACKAGE = {
  ...
  dependencies: {
    ...
    'bscncore-lib': 'https://github.com/BSCN-Project/bscncore-lib.git#master',
    'bscncore-node': 'https://github.com/BSCN-Project/bscncore-node.git#master'
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/bscncore-node.json', configJSON);
      if (!isGlobal) {
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the bscncore-node directory and configuration
