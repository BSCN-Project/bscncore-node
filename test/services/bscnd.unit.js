...
var Transaction = bscncore.Transaction;
var readFileSync = sinon.stub().returns(fs.readFileSync(path.resolve(__dirname, '../data/bscn.conf')));
var BscnService = proxyquire('../../lib/services/bscnd', {
	fs: {
		readFileSync: readFileSync
	}
});
var defaultBscnConf = fs.readFileSync(path.resolve(__dirname, '../data/default.bscn.conf'), 'utf8');

describe('Bscn Service', function() {
	...
	var baseConfig = {
		node: {
			network: bscncore.Networks.testnet
		},
		spawn: {
			datadir: 'testdir',
			exec: 'testpath'
	...
	describe('@constructor', function() {
		it('will create an instance', function() {
			var bscnd = new BscnService(baseConfig);
			should.exist(bscnd);
		});
		it('will create an instance without `new`', function() {
			var bscnd = BscnService(baseConfig);
			should.exist(bscnd);
		});
		it('will init caches', function() {
			var bscnd = new BscnService(baseConfig);

			should.exist(bscnd.utxosCache);
			should.exist(bscnd.txidsCache);
			should.exist(bscnd.balanceCache);
			should.exist(bscnd.summaryCache);
			should.exist(bscnd.transactionDetailedCache);
			should.exist(bscnd.accountInfo);

			should.exist(bscnd.transactionCache);
			should.exist(bscnd.rawTransactionCache);
			should.exist(bscnd.rawJsonTransactionCache);
			should.exist(bscnd.blockCache);
			should.exist(bscnd.blockJsonCache);
			should.exist(bscnd.blockSubsidyCache);
			should.exist(bscnd.rawBlockCache);
			should.exist(bscnd.blockHeaderCache);
			should.exist(bscnd.zmqKnownTransactions);
			should.exist(bscnd.zmqKnownBlocks);
			should.exist(bscnd.lastTip);
			should.exist(bscnd.lastTipTimeout);
			should.equal(bscnd.dgpInfoCache, null, 'should be null');
			should.equal(bscnd.miningInfoCache, null, 'should be null');
			should.equal(bscnd.stakingInfoCache, null, 'should be null');

			// limits
			should.equal(bscnd.maxTxids, 1000);
			should.equal(bscnd.maxTransactionHistory, 50);
			should.equal(bscnd.maxAddressesQuery, 10000);
			should.equal(bscnd.shutdownTimeout, 15000);

			// spawn restart setting
			should.equal(bscnd.spawnRestartTime, 5000);
			should.equal(bscnd.spawnStopTime, 10000);

			// try all interval
			should.equal(bscnd.tryAllInterval, 1000);
			should.equal(bscnd.startRetryInterval, 5000);

			// rpc limits
			should.equal(bscnd.transactionConcurrency, 5);

			// sync progress level when zmq subscribes to events
			should.equal(bscnd.zmqSubscribeProgress, 0.9999);
		});
		it('will init clients', function() {
			var bscnd = new BscnService(baseConfig);
			bscnd.nodes.should.deep.equal([]);
			bscnd.nodesIndex.should.equal(0);
			bscnd.nodes.push({ client: sinon.stub() });
			should.exist(bscnd.client);
		});
		it('will set subscriptions', function() {
			var bscnd = new BscnService(baseConfig);
			bscnd.subscriptions.should.deep.equal({
				address: {},
				rawtransaction: [],
				hashblock: [],
				balance: {},
	...
	describe('#_initDefaults', function() {
		it('will set transaction concurrency', function() {
			var bscnd = new BscnService(baseConfig);
			bscnd._initDefaults({ transactionConcurrency: 10 });
			bscnd.transactionConcurrency.should.equal(10);
			bscnd._initDefaults({});
			bscnd.transactionConcurrency.should.equal(5);
		});
	});

	describe('@dependencies', function() {
		it('will have no dependencies', function() {
			BscnService.dependencies.should.deep.equal([]);
		});
	});
	...
	describe('#getAPIMethods', function() {
		it('will return spec', function() {
			var bscnd = new BscnService(baseConfig);
			var methods = bscnd.getAPIMethods();
	...
	describe('#getPublishEvents', function() {
		it('will return spec', function() {
			var bscnd = new BscnService(baseConfig);
			var events = bscnd.getPublishEvents();
			should.exist(events);
			events.length.should.equal(4);

			events[0].name.should.equal('bscnd/rawtransaction');
			events[0].scope.should.equal(bscnd);
			events[0].subscribe.should.be.a('function');
			events[0].unsubscribe.should.be.a('function');

			events[1].name.should.equal('bscnd/hashblock');
			events[1].scope.should.equal(bscnd);
			events[1].subscribe.should.be.a('function');
			events[1].unsubscribe.should.be.a('function');

			events[2].name.should.equal('bscnd/addresstxid');
			events[2].scope.should.equal(bscnd);
			events[2].subscribe.should.be.a('function');
			events[2].unsubscribe.should.be.a('function');

			events[3].name.should.equal('bscnd/addressbalance');
			events[3].scope.should.equal(bscnd);
		...
		it('will call subscribe/unsubscribe with correct args', function() {
			var bscnd = new BscnService(baseConfig);
			bscnd.subscribe = sinon.stub();
			bscnd.unsubscribe = sinon.stub();
			var events = bscnd.getPublishEvents();

			events[0].subscribe('test');
			bscnd.subscribe.args[0][0].should.equal('rawtransaction');
			bscnd.subscribe.args[0][1].should.equal('test');

			events[0].unsubscribe('test');
			bscnd.unsubscribe.args[0][0].should.equal('rawtransaction');
			bscnd.unsubscribe.args[0][1].should.equal('test');

			events[1].subscribe('test');
			bscnd.subscribe.args[1][0].should.equal('hashblock');
			bscnd.subscribe.args[1][1].should.equal('test');

			events[1].unsubscribe('test');
			bscnd.unsubscribe.args[1][0].should.equal('hashblock');
			bscnd.unsubscribe.args[1][1].should.equal('test');
		});
	});
	...
	describe('#subscribe', function() {
		...
		it('will push to subscriptions', function() {
			var bscnd = new BscnService(baseConfig);
			var emitter = {};
			bscnd.subscribe('hashblock', emitter);
			bscnd.subscriptions.hashblock[0].should.equal(emitter);

			var emitter2 = {};
			bscnd.subscribe('rawtransaction', emitter2);
			bscnd.subscriptions.rawtransaction[0].should.equal(emitter2);
		});
	});
	...
	describe('#unsubscribe', function() {
		...
		it('will remove item from subscriptions', function() {
			...
			var emitter4 = {};
			var emitter5 = {};
			bscnd.subscribe('hashblock', emitter1);
			bscnd.subscribe('hashblock', emitter2);
			bscnd.subscribe('hashblock', emitter3);
			bscnd.subscribe('hashblock', emitter4);
			bscnd.subscribe('hashblock', emitter5);
			bscnd.subscriptions.hashblock.length.should.equal(5);

			bscnd.unsubscribe('hashblock', emitter3);
			bscnd.subscriptions.hashblock.length.should.equal(4);
			bscnd.subscriptions.hashblock[0].should.equal(emitter1);
			bscnd.subscriptions.hashblock[1].should.equal(emitter2);
			bscnd.subscriptions.hashblock[2].should.equal(emitter4);
			bscnd.subscriptions.hashblock[3].should.equal(emitter5);
		});
		it('will not remove item an already unsubscribed item', function() {
			var bscnd = new BscnService(baseConfig);
			var emitter1 = {};
			var emitter3 = {};
			bscnd.subscriptions.hashblock = [emitter1];
			bscnd.unsubscribe('hashblock', emitter3);
			bscnd.subscriptions.hashblock.length.should.equal(1);
			bscnd.subscriptions.hashblock[0].should.equal(emitter1);
		});
	});
	...
	describe('#subscribeAddress', function() {
		...
		it('will not an invalid address', function() {
			...
			bscnd.subscribeAddress(emitter, ['invalidaddress']);
			should.not.exist(bscnd.subscriptions.address['invalidaddress']);
		});
		it('will add a valid address', function() {
			var bscnd = new BscnService(baseConfig);
			var emitter = new EventEmitter();
			bscnd.subscribeAddress(emitter, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(bscnd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
		});
		it('will handle multiple address subscribers', function() {
			var bscnd = new BscnService(baseConfig);
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			bscnd.subscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			bscnd.subscribeAddress(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(bscnd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			bscnd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		});
		it('will not add the same emitter twice', function() {
			...
			bscnd.subscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(bscnd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			bscnd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
	});
	...
	describe('#unsubscribeAddress', function() {
		...
		it('it will remove a subscription', function() {
			...
			should.exist(bscnd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			bscnd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			bscnd.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			bscnd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
		it('will unsubscribe subscriptions for an emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			bscnd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			bscnd.unsubscribeAddress(emitter1);
			bscnd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing address', function() {
			...
			var emitter2 = new EventEmitter();
			bscnd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			bscnd.unsubscribeAddress(emitter1, ['qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX']);
			bscnd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		});
		it('will NOT unsubscribe subscription with missing emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			bscnd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter2];
			bscnd.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			bscnd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			bscnd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'][0].should.equal(emitter2);
		});
		it('will remove empty addresses', function() {
			...
			bscnd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			bscnd.unsubscribeAddress(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			bscnd.unsubscribeAddress(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.not.exist(bscnd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
		});
		it('will unsubscribe emitter for all addresses', function() {
			...
			var emitter2 = new EventEmitter();
			bscnd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			bscnd.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			sinon.spy(bscnd, 'unsubscribeAddressAll');
			bscnd.unsubscribeAddress(emitter1);
			bscnd.unsubscribeAddressAll.callCount.should.equal(1);
			bscnd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			bscnd.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
		});
	});
	...
	describe('#unsubscribeAddressAll', function() {
		...
		it('will unsubscribe emitter for all addresses', function() {
			...
			bscnd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			bscnd.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			bscnd.subscriptions.address['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'] = [emitter2];
			bscnd.subscriptions.address['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR'] = [emitter1];
			bscnd.unsubscribeAddressAll(emitter1);
			bscnd.subscriptions.address['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			bscnd.subscriptions.address['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			bscnd.subscriptions.address['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'].length.should.equal(1);
			should.not.exist(bscnd.subscriptions.address['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR']);
		});
	});
	...
	describe('#_getDefaultConfig', function() {
		it('will generate config file from defaults', function() {
			var bscnd = new BscnService(baseConfig);
			var config = bscnd._getDefaultConfig();
			config.should.equal(defaultBscnConf);
		});
	});
	...
	describe('#_loadSpawnConfiguration', function() {
		...
		it('will parse a bscn.conf file', function() {
			var TestBscn = proxyquire('../../lib/services/bscnd', {
				fs: {
					readFileSync: readFileSync,
			...
			var bscnd = new TestBscn(baseConfig);
			bscnd.options.spawn.datadir = '/tmp/.bscn';

			sinon.spy(bscnd, '_expandRelativeDatadir');
			sinon.spy(bscnd, '_checkConfigIndexes');
			sinon.spy(bscnd, '_getDefaultConf')
			sinon.spy(bscnd, '_getNetworkConfigPath');
			sinon.spy(bscnd, '_parseBitcoinConf');

			var node = {};
			bscnd._loadSpawnConfiguration(node);

			bscnd._expandRelativeDatadir.callCount.should.equal(1);
			bscnd._checkConfigIndexes.callCount.should.equal(1);
			bscnd._getDefaultConf.callCount.should.equal(1);
			bscnd._getNetworkConfigPath.callCount.should.equal(1);
			bscnd._parseBitcoinConf.callCount.should.equal(2);

			should.exist(bscnd.spawn.config);
			bscnd.spawn.config.should.deep.equal({
				addressindex: 1,
				checkblocks: 144,
				dbcache: 8192,
		...
		it('will expand relative datadir to absolute path', function() {
			...
			var config = {
				node: {
					network: bscncore.Networks.testnet,
					configPath: '/tmp/.bscncore/bscncore-node.json'
				},
				spawn: {
					datadir: './data',
					exec: 'testpath'
			...
			bscnd.options.spawn.datadir = './data';
			var node = {};
			bscnd._loadSpawnConfiguration(node);
			...
			bscnd.options.spawn.datadir.should.equal('/tmp/.bscncore/data');
		});
		it('should throw an exception if txindex isn\'t enabled in the configuration', function() {
			var TestBscn = proxyquire('../../lib/services/bscnd', {
				fs: {
					readFileSync: sinon.stub().returns(fs.readFileSync(__dirname + '/../data/badbscn.conf')),
					existsSync: sinon.stub().returns(true),
				},
				mkdirp: {
					sync: sinon.stub()
			...
			var bscnd = new TestBscn(baseConfig);
			(function() {
				bscnd._loadSpawnConfiguration({ datadir: './test' });
			}).should.throw(bscncore.errors.InvalidState);
		});
	...
	describe('#_checkConfigIndexes', function() {
		...
		it('should warn the user if txindex isn\'t set to 1 in the bscn.conf file', function() {
			var bscnd = new BscnService(baseConfig);
			var config = {
				txindex: 0,
				addressindex: 1,
			...
			var node = {};
			(function() {
				bscnd._checkConfigIndexes(config, node);
			}).should.throw('"txindex" option');
		});
		it('should warn the user if addressindex isn\'t set to 1 in the bscn.conf file', function() {
			var bscnd = new BscnService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 0,
		...
		it('should warn the user if spentindex isn\'t set to 1 in the bscn.conf file', function() {
			var bscnd = new BscnService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
		...
		it('should warn the user if server isn\'t set to 1 in the bscn.conf file', function() {
			var bscnd = new BscnService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
		...
		it('should warn the user if reindex is set to 1 in the bscn.conf file', function() {
			var bscnd = new BscnService(baseConfig);
			var config = {
				txindex: 1,
				addressindex: 1,
		...
		it('should warn if zmq port and hosts do not match', function() {
			...
			var config = {
				...
				server: 1,
				zmqpubrawtx: 'tcp://127.0.0.1:22501',
				zmqpubhashblock: 'tcp://127.0.0.1:22502',
				reindex: 1
			};
			var node = {};
			(function() {
	...
	describe('#_resetCaches', function() {
		it('will reset LRU caches', function() {
			...
			for (var i = 0; i < 10; i++) {
				...
				bscnd.transactionDetailedCache.set(keys[i], {});
				bscnd.utxosCache.set(keys[i], {});
				bscnd.accountInfo.set(keys[i], {});
				bscnd.txidsCache.set(keys[i], {});
				bscnd.balanceCache.set(keys[i], {});
				bscnd.summaryCache.set(keys[i], {});
				bscnd.blockOverviewCache.set(keys[i], {});
				bscnd.blockJsonCache.set(keys[i], {});
			}
			bscnd._resetCaches();
			should.equal(bscnd.transactionDetailedCache.get(keys[0]), undefined);
			should.equal(bscnd.utxosCache.get(keys[0]), undefined);
			should.equal(bscnd.accountInfo.get(keys[0]), undefined);
			should.equal(bscnd.txidsCache.get(keys[0]), undefined);
			should.equal(bscnd.balanceCache.get(keys[0]), undefined);
			should.equal(bscnd.summaryCache.get(keys[0]), undefined);
			should.equal(bscnd.blockOverviewCache.get(keys[0]), undefined);
			should.equal(bscnd.blockJsonCache.get(keys[0]), undefined);
	...
	describe('#_tryAllClients', function() {
		it('will retry for each node client', function(done) {
			...
			bscnd._tryAllClients(function(client, next) {
				client.getInfo(next);
			}, function(err) {
				if (err) {
					return done(err);
				}
				bscnd.nodes[0].client.getInfo.callCount.should.equal(1);
				bscnd.nodes[1].client.getInfo.callCount.should.equal(1);
				bscnd.nodes[2].client.getInfo.callCount.should.equal(1);
				done();
			});
		});
		it('will start using the current node index (round-robin)', function(done) {
			...
			bscnd._tryAllClients(function(client, next) {
				...
				bscnd.nodes[1].client.getInfo.callCount.should.equal(1);
				bscnd.nodes[2].client.getInfo.callCount.should.equal(1);
				bscnd.nodesIndex.should.equal(2);
				done();
			});
		});
	...
	describe('#_wrapRPCError', function() {
		it('will convert bscnd-rpc error object into JavaScript error', function() {
			var bscnd = new BscnService(baseConfig);
			var error = bscnd._wrapRPCError({ message: 'Test error', code: -1 });
	...
	describe('#_initChain', function() {
		...
		it('will set height and genesis buffer', function(done) {
			...
			bscnd.getRawBlock = sinon.stub().callsArgWith(1, null, genesisBuffer);
			bscnd.nodes.push({
				client: {
			...
			bscnd._initChain(function() {
				log.info.callCount.should.equal(1);
				bscnd.getRawBlock.callCount.should.equal(1);
				bscnd.getRawBlock.args[0][0].should.equal('genesishash');
				bscnd.height.should.equal(5000);
				bscnd.genesisBuffer.should.equal(genesisBuffer);
				done();
			});
		});
		...
		it('it will handle error from getRawBlock', function(done) {
			...
			bscnd.getRawBlock = sinon.stub().callsArgWith(1, new Error('test'));
			bscnd._initChain(function(err) {
				err.should.be.instanceOf(Error);
				getBestBlockHash.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
	...
	describe('#_getDefaultConf', function() {
		afterEach(function() {
			bscncore.Networks.disableRegtest();
			baseConfig.node.network = bscncore.Networks.testnet;
		});
		it('will get default rpc port for livenet', function() {
			var config = {
				node: {
					network: bscncore.Networks.livenet
				},
				spawn: {
					datadir: 'testdir',
					exec: 'testpath'
			...
			var bscnd = new BscnService(config);
			bscnd._getDefaultConf().rpcport.should.equal(22002);
		});
		it('will get default rpc port for testnet', function() {
			...
			var bscnd = new BscnService(config);
			bscnd._getDefaultConf().rpcport.should.equal(22302);
		});
	...
	describe('#_getNetworkConfigPath', function() {
		...
		it('will get default config path for livenet', function() {
			...
			var bscnd = new BscnService(config);
			should.equal(bscnd._getNetworkConfigPath(), undefined);
		});
		it('will get default rpc port for testnet', function() {
			var config = {
				node: {
			...
			var bscnd = new BscnService(config);
			bscnd._getNetworkConfigPath().should.equal('testnet3/bscn.conf');
		});
		it('will get default rpc port for regtest', function() {
			bscncore.Networks.enableRegtest();
			var config = {
			...
			var bscnd = new BscnService(config);
			bscnd._getNetworkConfigPath().should.equal('regtest/bscn.conf');
		});
	});
	...
	describe('#_getNetworkOption', function() {
		...
		it('return --testnet for testnet', function() {
			var bscnd = new BscnService(baseConfig);
			bscnd.node.network = bscncore.Networks.testnet;
			bscnd._getNetworkOption().should.equal('--testnet');
		});
		it('return --regtest for testnet', function() {
			var bscnd = new BscnService(baseConfig);
			bscnd.node.network = bscncore.Networks.testnet;
			bscncore.Networks.enableRegtest();
			bscnd._getNetworkOption().should.equal('--regtest');
		});
		it('return undefined for livenet', function() {
			var bscnd = new BscnService(baseConfig);
			bscnd.node.network = bscncore.Networks.livenet;
			bscncore.Networks.enableRegtest();
			should.equal(bscnd._getNetworkOption(), undefined);
		});
	});
	...
	describe('#_zmqBlockHandler', function() {
		it('will emit block', function(done) {
			...
			bscnd._rapidProtectedUpdateTip = sinon.stub();
			bscnd.on('block', function(block) {
				block.should.equal(message);
				done();
			});
			bscnd._zmqBlockHandler(node, message);
			bscnd._rapidProtectedUpdateTip.callCount.should.equal(1);
		});
		it('will not emit same block twice', function(done) {
			...
			var message = new Buffer('76843f1dfe455347d897e757ff5ff51a16a0f62b5d02a659c5680392de3a2b89', 'hex');
			bscnd._rapidProtectedUpdateTip = sinon.stub();
			sinon.spy(bscnd, 'emit');
			bscnd.on('block', function(block) {
				block.should.equal(message);
				bscnd.emit.callCount.should.equal(1);
				bscnd._rapidProtectedUpdateTip.callCount.should.equal(1);
				done();
			});
			bscnd._zmqBlockHandler(node, message);
		...
		it('will call function to update tip', function() {
			...
			bscnd._zmqBlockHandler(node, message);
			bscnd._rapidProtectedUpdateTip.callCount.should.equal(1);
			bscnd._rapidProtectedUpdateTip.args[0][0].should.equal(node);
			bscnd._rapidProtectedUpdateTip.args[0][1].should.equal(message);
		});
		it('will emit to subscribers', function(done) {
			...
			sinon.spy(bscnd, 'emit');
			var emitter = new EventEmitter();
			bscnd.subscriptions.hashblock.push(emitter);
			emitter.on('bscnd/hashblock', function(blockHash) {
				bscnd._rapidProtectedUpdateTip.callCount.should.equal(1);
				bscnd.emit.callCount.should.equal(1);
	...
	describe('#_rapidProtectedUpdateTip', function() {
		it('will limit tip updates with rapid calls', function(done) {
			...
			var count = 0;
			function repeat() {
				bscnd._rapidProtectedUpdateTip(node, message);
				count++;
				if (count < 50) {
					repeat();
				}
	...
	describe('#_updateTip', function() {
		...
		it('log and emit rpc error from get block', function(done) {
			var bscnd = new BscnService(baseConfig);
			bscnd.syncPercentage = sinon.stub();
			sinon.spy(bscnd, '_resetCaches');
			bscnd.on('error', function(err) {
				err.code.should.equal(-1);
				err.message.should.equal('Test error');
				node.client.getBlock.callCount.should.equal(1);
				bscnd._resetCaches.callCount.should.equal(1);
			...
			bscnd._updateTip(node, message);
		});
		it('emit synced if percentage is 100', function(done) {
			var bscnd = new BscnService(baseConfig);
			bscnd.syncPercentage = sinon.stub().callsArgWith(0, null, 100);
			bscnd.on('synced', function() {
				bscnd.syncPercentage.callCount.should.equal(1);
				node.client.getBlock.callCount.should.equal(1);
				done();
			});
			var node = {
		...
		it('NOT emit synced if percentage is less than 100', function(done) {
			var bscnd = new BscnService(baseConfig);
			bscnd.syncPercentage = sinon.stub().callsArgWith(0, null, 99);
			bscnd.on('synced', function() {
				bscnd.syncPercentage.callCount.should.equal(0);
				node.client.getBlock.callCount.should.equal(1);
		...
		it('log and emit error from syncPercentage', function(done) {
			var bscnd = new BscnService(baseConfig);
			bscnd.syncPercentage = sinon.stub().callsArgWith(0, new Error('test'));
			bscnd.on('error', function(err) {
				log.error.callCount.should.equal(1);
				bscnd.syncPercentage.callCount.should.equal(1);
				node.client.getBlock.callCount.should.equal(1);
		...
		it('reset caches and set height', function(done) {
			var bscnd = new BscnService(baseConfig);
			bscnd.syncPercentage = sinon.stub();
			bscnd._resetCaches = sinon.stub();
			bscnd.on('tip', function(height) {
				bscnd._resetCaches.callCount.should.equal(1);
				height.should.equal(10);
				bscnd.height.should.equal(10);
				done();
			});
			var node = {
				client: {
	...
	describe('#_getAddressesFromTransaction', function() {
		it('will get results using bscncore.Transaction', function() {
			var bscnd = new BscnService(baseConfig);
			var wif = 'cVTiUNavxbCzuoaQboDcgEhKYHhgXRaeQ8ZRoYkVXE9mxT1TcSrc';
			var privkey = bscncore.PrivateKey.fromWIF(wif);
			var inputAddress = privkey.toAddress(bscncore.Networks.testnet);
			var outputAddress = bscncore.Address('qMDYc9oXQK19dmLSCQNs5okRt8z4XeMr1X');
			var tx = bscncore.Transaction();
			tx.from({
				txid: '63ea7d22cb412283f4c6964a0fddebf3069f2e8a4761e86b5d0e2a39b0e84e5c',
				outputIndex: 0,
				script: bscncore.Script(inputAddress),
				address: inputAddress.toString(),
				satoshis: 5000000000
			});
			tx.to(outputAddress, 5000000000);
			tx.sign(privkey);
			var addresses = bscnd._getAddressesFromTransaction(tx);
		...
		it('will handle non-standard script types', function() {
			...
			tx.addOutput(bscncore.Transaction.Output({
				script: bscncore.Script('OP_TRUE'),
				satoshis: 5000000000
			}));
			var addresses = bscnd._getAddressesFromTransaction(tx);
		...
		it('will return unique values', function() {
			var bscnd = new BscnService(baseConfig);
			var tx = bscncore.Transaction();
			var address = bscncore.Address('qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX');
			tx.addOutput(bscncore.Transaction.Output({
				script: bscncore.Script(address),
				satoshis: 5000000000
			}));
			tx.addOutput(bscncore.Transaction.Output({
				script: bscncore.Script(address),
	...
	describe('#_notifyAddressTxidSubscribers', function() {
		it('will emit event if matching addresses', function(done) {
			...
			bscnd._getAddressesFromTransaction = sinon.stub().returns([address]);
			var emitter = new EventEmitter();
			bscnd.subscriptions.address[address] = [emitter];
			var txid = 'a9f5d9dc39bfff65c12d8a7a8263f57cc187d4c6cc62815d65186726bbc5ecb5';
			var transaction = {};
			emitter.on('bscnd/addresstxid', function(data) {
			...
			bscnd._notifyAddressTxidSubscribers(txid, transaction);
		});
	...
	describe('#_zmqTransactionHandler', function() {
		it('will emit to subscribers', function(done) {
			...
			var emitter = new EventEmitter();
			bscnd.subscriptions.rawtransaction.push(emitter);
			emitter.on('bscnd/rawtransaction', function(hex) {
				hex.should.be.a('string');
				hex.should.equal(expectedBuffer.toString('hex'));
				done();
			});
			var node = {};
			bscnd._zmqTransactionHandler(node, expectedBuffer);
		});
		it('will NOT emit to subscribers more than once for the same tx', function(done) {
			...
			sinon.spy(bscnd, 'emit');
			bscnd.subscriptions.rawtransaction.push(emitter);
			emitter.on('bscnd/rawtransaction', function() {
				done();
			});
			var node = {};
			bscnd._zmqTransactionHandler(node, expectedBuffer);
	...
	describe('#_checkSyncedAndSubscribeZmqEvents', function() {
		...
		it('log errors, update tip and subscribe to zmq events', function(done) {
			var bscnd = new BscnService(baseConfig);
			bscnd._updateTip = sinon.stub();
			bscnd._subscribeZmqEvents = sinon.stub();
			var blockEvents = 0;
			bscnd.on('block', function() {
				blockEvents++;
			});
			var getBestBlockHash = sinon.stub().callsArgWith(0, null, {
			...
			bscnd._checkSyncedAndSubscribeZmqEvents(node);
			setTimeout(function() {
				...
				bscnd._updateTip.callCount.should.equal(11);
				bscnd._subscribeZmqEvents.callCount.should.equal(1);
				done();
			}, 200);
		});
	...
	describe('#_subscribeZmqEvents', function() {
		it('will call subscribe on zmq socket', function() {
			...
			bscnd._subscribeZmqEvents(node);
		...
		it('will call relevant handler for rawtx topics', function(done) {
			var bscnd = new BscnService(baseConfig);
			bscnd._zmqTransactionHandler = sinon.stub();
			var node = {
			...
			node.zmqSubSocket.on('message', function() {
				bscnd._zmqTransactionHandler.callCount.should.equal(1);
				done();
			});
		...
		it('will call relevant handler for hashblock topics', function(done) {
			var bscnd = new BscnService(baseConfig);
			bscnd._zmqBlockHandler = sinon.stub();
			var node = {
				zmqSubSocket: new EventEmitter()
			};
			node.zmqSubSocket.subscribe = sinon.stub();
			bscnd._subscribeZmqEvents(node);
			node.zmqSubSocket.on('message', function() {
				bscnd._zmqBlockHandler.callCount.should.equal(1);
				done();
			});
		...
		it('will ignore unknown topic types', function(done) {
			...
			bscnd._subscribeZmqEvents(node);
			node.zmqSubSocket.on('message', function() {
				bscnd._zmqBlockHandler.callCount.should.equal(0);
				bscnd._zmqTransactionHandler.callCount.should.equal(0);
				done();
			});
	...
	describe('#_initZmqSubSocket', function() {
		it('will setup zmq socket', function() {
			...
			var bscnd = new BscnService(baseConfig);
			var node = {};
			bscnd._initZmqSubSocket(node, 'url');
	...
	describe('#_checkReindex', function() {
		...
		it('give error from client getblockchaininfo', function(done) {
			...
			bscnd._checkReindex(node, function(err) {
				should.exist(err);
		...
		it('will wait until sync is 100 percent', function(done) {
			...
			bscnd._checkReindex(node, function() {
	...
	describe('#_loadTipFromNode', function() {
		...
		it('will give rpc error from client getbestblockhash', function(done) {
			...
			bscnd._loadTipFromNode(node, function(err) {
				err.should.be.instanceof(Error);
	...
	describe('#_stopSpawnedProcess', function() {
		...
		it('it will kill process and resume', function(done) {
			...
			var TestBscnService = proxyquire('../../lib/services/bscnd', {
				fs: {
					readFile: readFile
				}
			});
			var bscnd = new TestBscnService(baseConfig);
			bscnd.spawnStopTime = 1;
			bscnd._process = {};
			bscnd._process.kill = sinon.stub();
			bscnd._stopSpawnedBitcoin(function(err) {
				if (err) {
					return done(err);
				}
				bscnd._process.kill.callCount.should.equal(1);
				log.warn.callCount.should.equal(1);
				done();
			});
		});
		it('it will attempt to kill process and resume', function(done) {
			...
			error2.code = 'ESRCH';
			bscnd._process.kill = sinon.stub().throws(error2);
			bscnd._stopSpawnedBitcoin(function(err) {
				if (err) {
					return done(err);
				}
	...
	describe('#_spawnChildProcess', function() {
		...
		it('will give error from spawn config', function(done) {
			var bscnd = new BscnService(baseConfig);
			bscnd._loadSpawnConfiguration = sinon.stub();
			bscnd._loadSpawnConfiguration = sinon.stub().throws(new Error('test'));
			bscnd._spawnChildProcess(function(err) {
				bscnd._loadSpawnConfiguration.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will give error from stopSpawnedBitcoin', function() {
			var bscnd = new BscnService(baseConfig);
			bscnd._loadSpawnConfiguration = sinon.stub();
			bscnd._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, new Error('test'));
			bscnd._spawnChildProcess(function(err) {
				bscnd._loadSpawnConfiguration.callCount.should.equal(1);
				bscnd._stopSpawnedBitcoin.callCount.should.equal(1);
				err.should.be.instanceOf(Error);
				err.message.should.equal('test');
			});
		});
		it('will exit spawn if shutdown', function() {
			...
			var bscnd = new TestBscnService(config);
			bscnd.spawn = {};
			bscnd._loadSpawnConfiguration = sinon.stub();
			bscnd._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, null);
			bscnd.node.stopping = true;
			bscnd._spawnChildProcess(function(err) {
				err.should.be.instanceOf(Error);
		...
		it('will include network with spawn command and init zmq/rpc on node', function(done) {
			...
			bscnd._loadSpawnConfiguration = sinon.stub();
			bscnd.spawn = {};
			bscnd.spawn.exec = 'testexec';
			bscnd.spawn.configPath = 'testdir/bscn.conf';
			bscnd.spawn.datadir = 'testdir';
			bscnd.spawn.config = {};
			bscnd.spawn.config.rpcport = 20001;
			bscnd.spawn.config.rpcuser = 'bscn';
			bscnd.spawn.config.rpcpassword = 'password';
			bscnd.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';

			bscnd._loadTipFromNode = sinon.stub().callsArgWith(1, null);
			bscnd._initZmqSubSocket = sinon.stub();
			bscnd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			bscnd._checkReindex = sinon.stub().callsArgWith(1, null);
			bscnd._spawnChildProcess(function(err, node) {
				should.not.exist(err);
				spawn.callCount.should.equal(1);
				spawn.args[0][0].should.equal('testexec');
				spawn.args[0][1].should.deep.equal([
					'--conf=testdir/bscn.conf',
					'--datadir=testdir',
					'--testnet'
				]);
				...
				bscnd._loadTipFromNode.callCount.should.equal(1);
				bscnd._initZmqSubSocket.callCount.should.equal(1);
				should.exist(bscnd._initZmqSubSocket.args[0][0].client);
				bscnd._initZmqSubSocket.args[0][1].should.equal('tcp://127.0.0.1:30001');
				bscnd._checkSyncedAndSubscribeZmqEvents.callCount.should.equal(1);
				should.exist(bscnd._checkSyncedAndSubscribeZmqEvents.args[0][0].client);
				should.exist(node);
				should.exist(node.client);
				done();
			});
		});
		it('will respawn bscnd spawned process', function(done) {
			var process = new EventEmitter();
			var spawn = sinon.stub().returns(process);
			var TestBscnService = proxyquire('../../lib/services/bscnd', {
				fs: {
			...
			bscnd._loadSpawnConfiguration = sinon.stub();
			bscnd.spawn = {};
			bscnd.spawn.exec = 'bscnd';
			bscnd.spawn.datadir = '/tmp/bscn';
			bscnd.spawn.configPath = '/tmp/bscn/bscn.conf';
			bscnd.spawn.config = {};
			bscnd.spawnRestartTime = 1;
			bscnd._loadTipFromNode = sinon.stub().callsArg(1);
			bscnd._initZmqSubSocket = sinon.stub();
			bscnd._checkReindex = sinon.stub().callsArg(1);
			bscnd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			bscnd._stopSpawnedBitcoin = sinon.stub().callsArg(0);
			sinon.spy(bscnd, '_spawnChildProcess');
			bscnd._spawnChildProcess(function(err) {
				if (err) {
					return done(err);
				}
				process.once('exit', function() {
					setTimeout(function() {
						bscnd._spawnChildProcess.callCount.should.equal(2);
						done();
					}, 5);
				});
				process.emit('exit', 1);
		...
		it('will emit error during respawn', function(done) {
			...
			bscnd._spawnChildProcess(function(err) {
				...
				bscnd._spawnChildProcess = sinon.stub().callsArgWith(0, new Error('test'));
				bscnd.on('error', function(err) {
					err.should.be.instanceOf(Error);
					err.message.should.equal('test');
					done();
		...
		it('will NOT respawn bscnd spawned process if shutting down', function(done) {
			var process = new EventEmitter();
			var spawn = sinon.stub().returns(process);
			var TestBscnService = proxyquire('../../lib/services/bscnd', {
				fs: {
			...
			bscnd._spawnChildProcess(function(err) {
				...
				process.once('exit', function() {
					setTimeout(function() {
						bscnd._spawnChildProcess.callCount.should.equal(1);
						done();
					}, 5);
				});
				process.emit('exit', 1);
		...
		it('will give error after 60 retries', function(done) {
			...
			bscnd.spawn.config.rpcpassword = 'password';
			bscnd.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';
			bscnd._loadTipFromNode = sinon.stub().callsArgWith(1, new Error('test'));
			bscnd._spawnChildProcess(function(err) {
				bscnd._loadTipFromNode.callCount.should.equal(60);
				err.should.be.instanceof(Error);
				done();
			});
		});
		it('will give error from check reindex', function(done) {
			...
			bscnd._initZmqSubSocket = sinon.stub();
			bscnd._checkSyncedAndSubscribeZmqEvents = sinon.stub();
			bscnd._checkReindex = sinon.stub().callsArgWith(1, new Error('test'));
	...
	describe('#_connectProcess', function() {
		it('will give error if connecting while shutting down', function(done) {
			...
			var bscnd = new BscnService(config);
			bscnd.node.stopping = true;
			bscnd.startRetryInterval = 100;
			bscnd._loadTipFromNode = sinon.stub();
			bscnd._connectProcess({}, function(err) {
				err.should.be.instanceof(Error);
				err.message.should.match(/Stopping while trying to connect to bscnd/);
				bscnd._loadTipFromNode.callCount.should.equal(0);
				done();
			});
		});
		it('will give error from loadTipFromNode after 60 retries', function(done) {
			...
			bscnd.startRetryInterval = 1;
			var config = {};
			bscnd._connectProcess(config, function(err) {
				err.should.be.instanceof(Error);
				bscnd._loadTipFromNode.callCount.should.equal(60);
				done();
			});
		...
		it('will init zmq/rpc on node', function(done) {
			...
			bscnd._loadTipFromNode = sinon.stub().callsArgWith(1, null);
			var config = {};
			bscnd._connectProcess(config, function(err, node) {
				should.not.exist(err);
				bscnd._loadTipFromNode.callCount.should.equal(1);
				bscnd._initZmqSubSocket.callCount.should.equal(1);
				bscnd._loadTipFromNode.callCount.should.equal(1);
	...
	describe('#start', function() {
		...
		it('will give error if "spawn" and "connect" are both not configured', function(done) {
			var bscnd = new BscnService(baseConfig);
			bscnd.options = {};
			bscnd.start(function(err) {
				err.should.be.instanceof(Error);
				err.message.should.be.a('string').and.equal('Bscn configuration options "spawn" or "connect" are expected');
				done();
			});
		});
		...
		it('will give error from connectProcess', function(done) {
			var bscnd = new BscnService(baseConfig);
			bscnd._connectProcess = sinon.stub().callsArgWith(1, new Error('test'));
			bscnd.options = {
				connect: [
					{}
				]
			...
			bscnd.start(function(err) {
				bscnd._connectProcess.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will push node from spawnChildProcess', function(done) {
			var bscnd = new BscnService(baseConfig);
			var node = {};
			bscnd._initChain = sinon.stub().callsArg(0);
			bscnd._spawnChildProcess = sinon.stub().callsArgWith(0, null, node);
			bscnd.options = {
				spawn: {}
			};
			bscnd.start(function(err) {
				should.not.exist(err);
				bscnd.nodes.length.should.equal(1);
				done();
			});
		});
		it('will push node from connectProcess', function(done) {
			...
			sinon.spy(bscnd, '_spawnChildProcess');
			var nodes = [{}];
			bscnd._connectProcess = sinon.stub().callsArgWith(1, null, nodes);
			bscnd.options = {
				connect: [
					{}
				]
			...
			bscnd.start(function(err) {
				should.not.exist(err);
				bscnd._initChain.callCount.should.equal(1);
				bscnd._connectProcess.callCount.should.equal(1);
				bscnd.nodes.length.should.equal(1);
				done();
			});
	...
	describe('#isSynced', function() {
		...
		it('will give "true" if percentage is 100.00', function(done) {
			var bscnd = new BscnService(baseConfig);
			bscnd.syncPercentage = sinon.stub().callsArgWith(0, null, 100.00);
			bscnd.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
				bscnd.syncPercentage.callCount.should.equal(1);
		...
		it('will give "true" if percentage is 99.98', function(done) {
			var bscnd = new BscnService(baseConfig);
			bscnd.syncPercentage = sinon.stub().callsArgWith(0, null, 99.98);
			bscnd.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
		...
		it('will give "false" if percentage is 99.49', function(done) {
			var bscnd = new BscnService(baseConfig);
			bscnd.syncPercentage = sinon.stub().callsArgWith(0, null, 99.49);
			bscnd.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
		...
		it('will give "false" if percentage is 1', function(done) {
			var bscnd = new BscnService(baseConfig);
			bscnd.syncPercentage = sinon.stub().callsArgWith(0, null, 1);
			bscnd.isSynced(function(err, synced) {
				if (err) {
					return done(err);
				}
	...
	describe('#syncPercentage', function() {
		it('will give rpc error', function(done) {
			...
			bscnd.syncPercentage(function(err) {
				should.exist(err);
				bscnd.nodes[0].client.getBlockchainInfo.callCount.should.equal(1);
		...
		it('will call client getInfo and give result', function(done) {
			...
			bscnd.syncPercentage(function(err, percentage) {
				if (err) {
					return done(err);
				}
				bscnd.nodes[0].client.getBlockchainInfo.callCount.should.equal(1);
	...
	describe('#_normalizeAddressArg', function() {
		it('will turn single address into array', function() {
			var bscnd = new BscnService(baseConfig);
			var args = bscnd._normalizeAddressArg('address');
			args.should.deep.equal(['address']);
		});
		it('will keep an array as an array', function() {
			var bscnd = new BscnService(baseConfig);
			var args = bscnd._normalizeAddressArg(['address one', 'address two']);
	...
	describe('#getAddressBalance', function() {
		it('will give rpc error', function(done) {
			var bscnd = new BscnService(baseConfig);
			sinon.spy(bscnd, '_normalizeAddressArg');
			bscnd.nodes.push({
				client: {
			...
			var options = {};
			bscnd.getAddressBalance(address, options, function(err) {
				bscnd._normalizeAddressArg.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				done();
			});
		});
		it('will give balance', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			var options = {};
			bscnd.getAddressBalance(address, options, function(err, data) {
				if (err) {
					return done(err);
				}
				data.balance.should.equal(10000);
				data.received.should.equal(100000);
				bscnd.getAddressBalance(address, options, function(err, data2) {
					if (err) {
						return done(err);
					}
					data2.balance.should.equal(10000);
					data2.received.should.equal(100000);
					bscnd._normalizeAddressArg.callCount.should.equal(2);
	...
	describe('#getAddressUnspentOutputs', function() {
		it('will give rpc error', function(done) {
			...
			bscnd.getAddressUnspentOutputs(address, options, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		...
		it('will give results from client getaddressutxos', function(done) {
			...
			var address = 'qMYemSxspKpzB2RbDunJRosQv5PJd3eEWD';
			bscnd.getAddressUnspentOutputs(address, options, function(err, utxos) {
				if (err) {
					return done(err);
				}
	...
	describe('#_getBalanceFromMempool', function() {
		it('will sum satoshis', function() {
			...
			var sum = bscnd._getBalanceFromMempool(deltas);
	...
	describe('#_getTxidsFromMempool', function() {
		it('will filter to txids', function() {
			...
			var txids = bscnd._getTxidsFromMempool(deltas);
	...
	describe('#_getHeightRangeQuery', function() {
		it('will detect range query', function() {
			...
			var rangeQuery = bscnd._getHeightRangeQuery(options);
		...
		it('will get range properties', function() {
			...
			var clone = {};
			var rangeQuery = bscnd._getHeightRangeQuery(options, clone);
		...
		it('will throw error with invalid range', function() {
			...
			(function() {
				bscnd._getHeightRangeQuery(options);
	...
	describe('#getAddressTxids', function() {
		it('will give error from _getHeightRangeQuery', function(done) {
			var bscnd = new BscnService(baseConfig);
			bscnd._getHeightRangeQuery = sinon.stub().throws(new Error('test'));
			bscnd.getAddressTxids('address', {}, function(err) {
				err.should.be.instanceOf(Error);
				err.message.should.equal('test');
				done();
			});
		...
		it('will give rpc error from mempool query', function() {
			...
			sinon.spy(bscnd, '_getHeightRangeQuery');
			sinon.spy(bscnd, '_normalizeAddressArg');
			...
			var options = {};
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			bscnd.getAddressTxids(address, options, function(err) {
				should.exist(err);
				bscnd._getHeightRangeQuery.callCount.should.equal(1);
				bscnd._normalizeAddressArg.callCount.should.equal(1);
				err.should.be.instanceof(errors.RPCError);
			});
		});
		...
		it('will get txid results', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			bscnd.getAddressTxids(address, options, function(err, txids) {
				if (err) {
					return done(err);
				}
				bscnd._getHeightRangeQuery.callCount.should.equal(1);
		...
		it('will get txid results from cache', function(done) {
			...
			bscnd.getAddressTxids(address, options, function(err, txids) {
				...
				bscnd.getAddressTxids(address, options, function(err, txids) {
					...
					bscnd._getHeightRangeQuery.callCount.should.equal(2);
					bscnd._normalizeAddressArg.callCount.should.equal(2);
					getAddressTxids.callCount.should.equal(1);
					txids.should.deep.equal(expectedTxids);
					done();
		...
		it('will get txid results WITHOUT cache if rangeQuery and exclude mempool', function(done) {
			...
			bscnd.getAddressTxids(address, options, function(err, txids) {
				...
				bscnd.getAddressTxids(address, options, function(err, txids) {
					...
					bscnd._getHeightRangeQuery.callCount.should.equal(4);
					bscnd._normalizeAddressArg.callCount.should.equal(2);
		...
		it('will get txid results from cache and live mempool', function(done) {
			...
			var address = 'qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2';
			bscnd.getAddressTxids(address, { queryMempool: false }, function(err, txids) {
				if (err) {
					return done(err);
				}
				getAddressTxids.callCount.should.equal(1);
				...
				bscnd.getAddressTxids(address, { queryMempool: true }, function(err, txids) {
					if (err) {
						return done(err);
					}
					getAddressTxids.callCount.should.equal(1);
					...
					bscnd.getAddressTxids(address, { queryMempoolOnly: true }, function(err, txids) {
						if (err) {
							return done(err);
						}
						getAddressTxids.callCount.should.equal(1);
						...
						bscnd._getHeightRangeQuery.callCount.should.equal(3);
						bscnd._normalizeAddressArg.callCount.should.equal(3);
						done();
					});
				});
			});
	...
	describe('#_getConfirmationDetail', function() {
		...
		it('should get 0 confirmation', function() {
			...
			var bscnd = new BscnService(baseConfig);
			bscnd.height = 10;
			var confirmations = bscnd._getConfirmationsDetail(tx);
	...
	describe('#_getAddressDetailsForInput', function() {
		it('will return if missing an address', function() {
			var bscnd = new BscnService(baseConfig);
			var result = {};
			bscnd._getAddressDetailsForInput({}, 0, result, []);
		...
		it('will only add address if it matches', function() {
			var bscnd = new BscnService(baseConfig);
			var result = {};
			bscnd._getAddressDetailsForInput({
				address: 'address1'
			}, 0, result, ['address2']);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
	...
	describe('#_getAddressDetailsForOutput', function() {
		it('will return if missing an address', function() {
			var bscnd = new BscnService(baseConfig);
			var result = {};
			bscnd._getAddressDetailsForOutput({}, 0, result, []);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
		});
		it('will only add address if it matches', function() {
			var bscnd = new BscnService(baseConfig);
			var result = {};
			bscnd._getAddressDetailsForOutput({
				address: 'address1'
			}, 0, result, ['address2']);
			should.not.exist(result.addresses);
			should.not.exist(result.satoshis);
	...
	describe('#_getAddressDetailsForTransaction', function() {
		it('will calculate details for the transaction', function(done) {
			...
			sinon.spy(bscnd, '_getAddressDetailsForInput');
			sinon.spy(bscnd, '_getAddressDetailsForOutput');

			var addresses = ['qey8KJ8xkyLUUqFZZwqngXX9U2q9ef6Tw4'];
			var details = bscnd._getAddressDetailsForTransaction(tx, addresses);
			...
			bscnd._getAddressDetailsForInput.callCount.should.equal(1);
			bscnd._getAddressDetailsForOutput.callCount.should.equal(5);
			done();
		});
	});
	...
	describe('#_getAddressDetailedTransaction', function() {
		it('will get detailed transaction info', function(done) {
			...
			var bscnd = new BscnService(baseConfig);
			sinon.spy(bscnd, '_getConfirmationsDetail');
			bscnd.getDetailedTransaction = sinon.stub().callsArgWith(1, null, tx);
			bscnd.height = 300;
			var addresses = {};
			bscnd._getAddressDetailsForTransaction = sinon.stub().returns({
				addresses: addresses,
				satoshis: 1000,
			});
			bscnd._getAddressDetailedTransaction(txid, {}, function(err, details) {
				if (err) {
					return done(err);
				}
				details.addresses.should.equal(addresses);
				details.satoshis.should.equal(1000);
				bscnd.getDetailedTransaction.callCount.should.equal(1);
				bscnd._getAddressDetailsForTransaction.callCount.should.equal(1);
				bscnd._getConfirmationsDetail.callCount.should.equal(1);
		...
		it('give error from _getDetailedTransaction', function(done) {
			...
			var bscnd = new BscnService(baseConfig);
			bscnd.getDetailedTransaction = sinon.stub().callsArgWith(1, new Error('test'));
			bscnd._getAddressDetailedTransaction(txid, {}, function(err) {
				err.should.be.instanceof(Error);
				bscnd.getDetailedTransaction.callCount.should.equal(1);
				done();
			});
	...
	describe('#_getAddressStrings', function() {
		it('will get address strings from bscncore addresses', function() {
			var addresses = [
				bscncore.Address('qU12Fa5RHM535kSDvywxPjCmbL7gwkQJZ6'),
				bscncore.Address('qcSLSxN1sngCWSrKFZ6UC7ri4hhVSdq9SU'),
			];
			var bscnd = new BscnService(baseConfig);
			var strings = bscnd._getAddressStrings(addresses);
		...
		it('will give error with unknown', function() {
			...
			var bscnd = new BscnService(baseConfig);
			(function() {
				bscnd._getAddressStrings(addresses);
	...
	describe('#getAddressHistory', function() {
		...
		it('will give an error if length of addresses is too long', function(done) {
			...
			var bscnd = new BscnService(baseConfig);
			sinon.spy(bscnd, '_normalizeAddressArg');
			bscnd.maxAddressesQuery = 100;
			bscnd.getAddressHistory(addresses, {}, function(err) {
				should.exist(err);
				bscnd._normalizeAddressArg.callCount.should.equal(1);
		...
		it('will give error with "from" and "to" range that exceeds max size', function(done) {
			var bscnd = new BscnService(baseConfig);
			sinon.spy(bscnd, '_normalizeAddressArg');
			sinon.spy(bscnd, '_getAddressStrings');
			bscnd.getAddressHistory(address, { from: 0, to: 51 }, function(err) {
				should.exist(err);
				bscnd._normalizeAddressArg.callCount.should.equal(1);
				bscnd._getAddressStrings.callCount.should.equal(1);
		...
		it('will give error with "from" and "to" order is reversed', function(done) {
			...
			sinon.spy(bscnd, '_normalizeAddressArg');
			sinon.spy(bscnd, '_getAddressStrings');
			bscnd.getAddressTxids = sinon.stub().callsArgWith(2, null, []);
			bscnd.getAddressHistory(address, { from: 51, to: 0 }, function(err) {
				should.exist(err);
				bscnd._normalizeAddressArg.callCount.should.equal(1);
				bscnd._getAddressStrings.callCount.should.equal(1);
				err.message.match(/^\"from/);
		...
		it('will give error from _getAddressDetailedTransaction', function(done) {
			...
			sinon.spy(bscnd, '_normalizeAddressArg');
			sinon.spy(bscnd, '_getAddressStrings');
			bscnd.getAddressTxids = sinon.stub().callsArgWith(2, null, ['txid']);
			bscnd._getAddressDetailedTransaction = sinon.stub().callsArgWith(2, new Error('test'));
			bscnd.getAddressHistory(address, {}, function(err) {
				should.exist(err);
				bscnd._normalizeAddressArg.callCount.should.equal(1);
				bscnd._getAddressStrings.callCount.should.equal(1);
				bscnd.getAddressTxids.callCount.should.equal(1);
				bscnd._getAddressDetailedTransaction.callCount.should.equal(1);
				err.message.should.equal('test');
				done();
			});
		});
		it('give error from getAddressTxids', function(done) {
			...
			sinon.spy(bscnd, '_normalizeAddressArg');
			sinon.spy(bscnd, '_getAddressStrings');
			sinon.spy(bscnd, 'getAddressTxids');
			bscnd.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
			bscnd.getAddressHistory('address', {}, function(err) {
				should.exist(err);
				bscnd._normalizeAddressArg.callCount.should.equal(1);
				bscnd._getAddressStrings.callCount.should.equal(1);
		...
		it('will paginate', function(done) {
			...
			sinon.spy(bscnd, '_normalizeAddressArg');
			sinon.spy(bscnd, '_getAddressStrings');
			sinon.spy(bscnd, '_paginateTxids');
			bscnd._getAddressDetailedTransaction = function(txid, options, callback) {
				callback(null, txid);
			};
			sinon.spy(bscnd, '_getAddressDetailedTransaction');
			var txids = ['one', 'two', 'three', 'four'];
			bscnd.getAddressTxids = sinon.stub().callsArgWith(2, null, txids);
			bscnd.getAddressHistory('address', { from: 1, to: 3 }, function(err, data) {
				if (err) {
					return done(err);
				}
				bscnd._normalizeAddressArg.callCount.should.equal(1);
				bscnd._getAddressStrings.callCount.should.equal(1);
				bscnd.getAddressTxids.callCount.should.equal(1);
				bscnd._getAddressDetailedTransaction.callCount.should.equal(2);
				bscnd._paginateTxids.callCount.should.equal(1);
	...
	describe('#getAddressSummary', function() {
		...
		it('will handle error from getAddressTxids', function(done) {
			...
			bscnd.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
			bscnd.getAddressBalance = sinon.stub().callsArgWith(2, null, {});
			var address = '';
			var options = {};
			bscnd.getAddressSummary(address, options, function(err) {
				should.exist(err);
				err.should.be.instanceof(Error);
				err.message.should.equal('test');
				done();
		...
		it('will handle error from getAddressBalance', function(done) {
			...
			bscnd.getAddressTxids = sinon.stub().callsArgWith(2, null, {});
			bscnd.getAddressBalance = sinon.stub().callsArgWith(2, new Error('test'), {});
			var address = '';
			var options = {};
			bscnd.getAddressSummary(address, options, function(err) {
				should.exist(err);
		...
		it('should set all properties', function(done) {
			...
			bscnd.getAddressTxids = sinon.stub().callsArgWith(2, null, [txid1, txid2, txid3]);
			bscnd.getAddressBalance = sinon.stub().callsArgWith(2, null, {
				received: 30 * 1e8,
				balance: 20 * 1e8
			});
			...
			var options = {};
			bscnd.getAddressSummary(address, options, function(err, summary) {
				bscnd._paginateTxids.callCount.should.equal(1);
				bscnd._normalizeAddressArg.callCount.should.equal(1);
				bscnd.getAddressTxids.callCount.should.equal(1);
				bscnd.getAddressBalance.callCount.should.equal(1);
				bscnd._paginateTxids.args[0][1].should.equal(0);
				bscnd._paginateTxids.args[0][2].should.equal(1000);
		...
		it('will get from cache with noTxList', function(done) {
			...
			bscnd.getAddressSummary(address, options, function(err, summary) {
				...
				bscnd.getAddressSummary(address, options, function(err, summary) {
					checkSummary(summary);
					bscnd._paginateTxids.callCount.should.equal(0);
					bscnd._normalizeAddressArg.callCount.should.equal(2);
					bscnd.getAddressTxids.callCount.should.equal(1);
					bscnd.getAddressBalance.callCount.should.equal(1);
					done();
		...
		it('will skip querying the mempool with queryMempool set to false', function(done) {
			...
			bscnd.getAddressSummary(address, options, function() {
				getAddressMempool.callCount.should.equal(0);
				bscnd._paginateTxids.callCount.should.equal(1);
				bscnd._normalizeAddressArg.callCount.should.equal(1);
				bscnd.getAddressTxids.callCount.should.equal(1);
		...
		it('will give error from _paginateTxids', function(done) {
			...
			bscnd._paginateTxids = sinon.stub().throws(new Error('test'));
			var address = 'qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR';
			var options = {
				queryMempool: false
			};
	...
	describe('#getRawBlock', function() {
		...
		it('will give rpc error from client getblockhash', function(done) {
			...
			bscnd.getRawBlock(10, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		...
		it('will give rcp error from client getblock', function(done) {
			...
			sinon.spy(bscnd, '_tryAllClients');
			bscnd.nodes.push({
				client: {
					getBlock: sinon.stub().callsArgWith(2, { code: -1, message: 'Test error' })
				}
			});
			bscnd.getRawBlock(blockhash, function(err) {
				should.exist(err);
				bscnd._tryAllClients.callCount.should.equal(1);
				err.should.be.instanceof(errors.RPCError);
				done();
			});
		});
		it('will try all nodes for getblock', function(done) {
			...
			bscnd.getRawBlock(blockhash, function(err, buffer) {
				if (err) {
					return done(err);
				}
				buffer.should.be.instanceof(Buffer);
		...
		it('will get block by height', function(done) {
			...
			bscnd.getRawBlock(0, function(err, buffer) {
				if (err) {
					return done(err);
				}
				buffer.should.be.instanceof(Buffer);
	...
	describe('#getBlock', function() {
		...
		it('will give an rpc error from client getblock', function(done) {
			...
			bscnd.getBlock(0, function(err) {
				bscnd._tryAllClients.callCount.should.equal(2);
				getBlock.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
				done();
		...
		it('will getblock as bscncore object from height', function(done) {
			var bscnd = new BscnService(baseConfig);
			sinon.spy(bscnd, '_tryAllClients');
			var getBlock = sinon.stub().callsArgWith(2, null, {
				result: blockhex
			...
			bscnd.getBlock(0, function(err, block) {
				should.not.exist(err);
				bscnd._tryAllClients.callCount.should.equal(2);
				getBlock.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				...
				block.should.be.instanceof(bscncore.Block);
				done();
			});
		});
		it('will getblock as bscncore object', function(done) {
			var bscnd = new BscnService(baseConfig);
			sinon.spy(bscnd, '_tryAllClients');
			var getBlock = sinon.stub().callsArgWith(2, null, {
				result: blockhex
			...
			bscnd.getBlock('00000000050a6d07f583beba2d803296eb1e9d4980c4a20f206c584e89a4f02b', function(err, block) {
				should.not.exist(err);
				bscnd._tryAllClients.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
		...
		it('will get block from cache', function(done) {
			...
			bscnd.getBlock(hash, function(err, block) {
				should.not.exist(err);
				bscnd._tryAllClients.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
				getBlock.callCount.should.equal(1);
		...
		it('will get block from cache with height (but not height)', function(done) {
			...
			bscnd.getBlock(0, function(err, block) {
				...
				bscnd.getBlock(0, function(err, block) {
					should.not.exist(err);
					bscnd._tryAllClients.callCount.should.equal(3);
	...
	describe('#getBlockHashesByTimestamp', function() {
		it('should give an rpc error', function(done) {
			...
			bscnd.getBlockHashesByTimestamp(1517410000, 1517843954, function(err, hashes) {
				should.exist(err);
	...
	describe('#getBlockHeader', function() {
		...
		it('will give error from getBlockHash', function() {
			var bscnd = new BscnService(baseConfig);
			sinon.spy(bscnd, '_maybeGetBlockHash');
			var getBlockHash = sinon.stub().callsArgWith(1, { code: -1, message: 'Test error' });
			bscnd.nodes.push({
				client: {
					getBlockHash: getBlockHash
			...
			bscnd.getBlockHeader(10, function(err) {
				bscnd._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('it will give rpc error from client getblockheader', function() {
			...
			bscnd.getBlockHeader(blockhash, function(err) {
				bscnd._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('it will give rpc error from client getblockhash', function() {
			...
			bscnd.getBlockHeader(0, function(err) {
				bscnd._maybeGetBlockHash.callCount.should.equal(1);
				err.should.be.instanceof(Error);
			});
		});
		it('will give result from client getblockheader (from height)', function() {
			...
			bscnd.getBlockHeader(81419, function(err, blockHeader) {
				should.not.exist(err);
				bscnd._maybeGetBlockHash.callCount.should.equal(1);
		...
		it('will give result from client getblockheader (from hash)', function() {
			...
			bscnd.getBlockHeader(blockhash, function(err, blockHeader) {
				should.not.exist(err);
				bscnd._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
				blockHeader.should.deep.equal(result);
	...
	describe('#_maybeGetBlockHash', function() {
		it('will not get block hash with an address', function(done) {
			...
			bscnd._maybeGetBlockHash('qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX', function(err, hash) {
				if (err) {
					return done(err);
				}
				bscnd._tryAllClients.callCount.should.equal(0);
				getBlockHash.callCount.should.equal(0);
		...
		it('will not get block hash with non zero-nine numeric string', function(done) {
			...
			bscnd._maybeGetBlockHash('109a', function(err, hash) {
				if (err) {
					return done(err);
				}
				getBlockHash.callCount.should.equal(0);
		...
		it('will get the block hash if argument is a number', function(done) {
			...
			bscnd._maybeGetBlockHash(10, function(err, hash) {
				if (err) {
					return done(err);
				}
	...
	describe('#getBlockOverview', function() {
		...
		it('will handle error from maybeGetBlockHash', function(done) {
			var bscnd = new BscnService(baseConfig);
			sinon.spy(bscnd, '_maybeGetBlockHash');
			bscnd._maybeGetBlockHash = sinon.stub().callsArgWith(1, new Error('test'));
			bscnd.getBlockOverview(blockhash, function(err) {
				err.should.be.instanceOf(Error);
				bscnd._maybeGetBlockHash.callCount.should.equal(1);
				done();
			});
		...
		it('will give expected result', function(done) {
			...
			bscnd.getBlockOverview(blockhash, function(err, blockOverview) {
				if (err) {
					return done(err);
				}
				...
				bscnd.getBlockOverview(blockhash, function(err, blockOverview) {
					checkBlock(blockOverview);
					bscnd._maybeGetBlockHash.callCount.should.equal(2);
					getBlock.callCount.should.equal(1);
					done();
				});
			});
	...
	describe('#estimateFee', function() {
		it('will give rpc error', function(done) {
			...
			bscnd.estimateFee(1, function(err) {
				should.exist(err);
		...
		it('will call client estimateFee and give result', function(done) {
			...
			bscnd.estimateFee(1, function(err, feesPerKb) {
				if (err) {
					return done(err);
				}
				estimateFee.callCount.should.equal(1);
	...
	describe('#sendTransaction', function(done) {
		var tx = bscncore.Transaction(txhex);
		it('will give rpc error', function() {
			...
			bscnd.sendTransaction(txhex, function(err) {
				should.exist(err);
		...
		it('will send to client and get hash', function() {
			...
			bscnd.sendTransaction(txhex, function(err, hash) {
				if (err) {
					return done(err);
				}
				sendRawTransaction.callCount.should.equal(1);
		...
		it('will send to client with absurd fees and get hash', function() {
			...
			bscnd.sendTransaction(txhex, { allowAbsurdFees: true }, function(err, hash) {
				if (err) {
					return done(err);
				}
				sendRawTransaction.callCount.should.equal(1);
		...
		it('missing callback will throw error', function() {
			...
			var transaction = bscncore.Transaction();
			(function() {
				bscnd.sendTransaction(transaction);
			}).should.throw(Error);
			sendRawTransaction.callCount.should.equal(1);
		});
	});
	...
	describe('#getRawTransaction', function() {
		it('will give rpc error', function(done) {
			...
			bscnd.getRawTransaction('txid', function(err) {
				should.exist(err);
				bscnd._tryAllClients.callCount.should.equal(1);
		...
		it('will try all nodes', function(done) {
			...
			bscnd.getRawTransaction('txid', function(err, tx) {
				if (err) {
					return done(err);
				}
				should.exist(tx);
	...
	describe('#getTransaction', function() {
		it('will give rpc error', function(done) {
			...
			bscnd.getTransaction('txid', function(err) {
				should.exist(err);
				bscnd._tryAllClients.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
				done();
		...
		it('will try all nodes', function(done) {
			...
			bscnd.getTransaction('txid', function(err, tx) {
				if (err) {
					return done(err);
				}
				should.exist(tx);
				...
				getRawTransaction.callCount.should.equal(1);
				bscnd._tryAllClients.callCount.should.equal(1);
				tx.should.be.an.instanceof(bscncore.Transaction);
				done();
			});
		});
		it('will get from cache', function(done) {
	...
	describe('#getDetailedTransaction', function() {
		...
		it('will give rpc error', function(done) {
			...
			bscnd.getDetailedTransaction(txid, function(err) {
				should.exist(err);
				err.should.be.instanceof(errors.RPCError);
				bscnd._tryAllClients.callCount.should.equal(1);
				done();
		...
		it('should give a transaction with all properties', function(done) {
			...
			bscnd.getDetailedTransaction(txid, function(err, tx) {
				if (err) {
					return done(err);
				}
				checkTx(tx);
	...
	describe('#getBestBlockHash', function() {
		it('will give rpc error', function(done) {
			...
			bscnd.getBestBlockHash(function(err) {
				should.exist(err);
				getBestBlockHash.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
				done();
		...
		it('will call client getInfo and give result', function(done) {
			...
			bscnd.getBestBlockHash(function(err, hash) {
				if (err) {
					return done(err);
				}
				getBestBlockHash.callCount.should.equal(1);
	...
	describe('#getSpentInfo', function() {
		it('will give rpc error', function(done) {
			...
			bscnd.getSpentInfo({}, function(err) {
				should.exist(err);
		...
		it('will empty object when not found', function(done) {
			...
			bscnd.getSpentInfo({}, function(err, info) {
				should.not.exist(err);
				getSpentInfo.callCount.should.equal(1);
	...
	describe('#getInfo', function() {
		...
		it('will call client getInfo and give result', function(done) {
			var bscnd = new BscnService(baseConfig);
			bscnd.node.getNetworkName = sinon.stub().returns('testnet');
			...
			bscnd.getSubsidy = sinon.stub().callsArgWith(1, null, 400000000);
			bscnd.nodes.push({
				client: {
					getInfo: getInfo,
				},
			...
			bscnd.getInfo(function(err, info) {
				if (err) {
					return done(err);
				}

				bscnd.getSubsidy.callCount.should.equal(1);
				bscnd.client.getInfo.callCount.should.equal(1);
	...
	describe('#generateBlock', function() {
		it('will give rpc error', function(done) {
			...
			bscnd.generateBlock(10, function(err) {
				should.exist(err);
		...
		it('will call client generate and give result', function(done) {
			...
			bscnd.generateBlock(10, function(err, hashes) {
				if (err) {
					return done(err);
				}
	...
	describe('#stop', function() {
		...
		it('will exit spawned process', function(done) {
			var bscnd = new BscnService(baseConfig);
			bscnd.spawn = {};
			bscnd.spawn.process = new EventEmitter();
			bscnd.spawn.process.kill = sinon.stub();
			bscnd.stop(done);
			bscnd.spawn.process.kill.callCount.should.equal(1);
			bscnd.spawn.process.kill.args[0][0].should.equal('SIGINT');
			bscnd.spawn.process.emit('exit', 0);
		});
		it('will give error with non-zero exit status code', function(done) {
			...
			bscnd.spawn.process.kill.callCount.should.equal(1);
			bscnd.spawn.process.kill.args[0][0].should.equal('SIGINT');
			bscnd.spawn.process.emit('exit', 1);
		});
		it('will stop after timeout', function(done) {
			var bscnd = new BscnService(baseConfig);
			bscnd.shutdownTimeout = 300;
			bscnd.spawn = {};
			bscnd.spawn.process = new EventEmitter();
			bscnd.spawn.process.kill = sinon.stub();
			bscnd.stop(function(err) {
	...
	describe('#getAddressesMempoolBalance', function() {
		it('will give rpc error', function(done) {
			...
			bscnd.getAddressesMempoolBalance([], {}, function(err, result) {
				should.exist(err);
				bscnd._normalizeAddressArg.callCount.should.equal(1);
				getAddressMempool.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will calc mempool balance', function(done) {
			...
			var bscnd = new BscnService(baseConfig);
			sinon.spy(bscnd, '_normalizeAddressArg');
			sinon.spy(bscnd, '_getBalanceFromMempool');
			var getAddressMempool = sinon.stub().callsArgWith(1, null, {
				result: deltas
			});
			bscnd.nodes.push({
			...
			bscnd.getAddressesMempoolBalance([], {}, function(err, result) {
				should.not.exist(err);
				bscnd._getBalanceFromMempool.callCount.should.equal(1);
				bscnd._normalizeAddressArg.callCount.should.equal(1);
				getAddressMempool.callCount.should.equal(1);
	...
	describe('#getJsonRawTransaction', function() {
		it('will give rpc error', function(done) {
			...
			bscnd.getJsonRawTransaction('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', function(err) {
				should.exist(err);
				bscnd._tryAllClients.callCount.should.equal(1);
				getRawTransaction.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give transaction', function(done) {
			...
			bscnd.getJsonRawTransaction('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', function(err, response) {
				should.not.exist(err);
				bscnd._tryAllClients.callCount.should.equal(1);
				getRawTransaction.callCount.should.equal(1);
	...
	describe('#getSubsidy', function() {
		it('will give rpc error', function(done) {
			...
			bscnd.getSubsidy(0, function(err) {
				should.exist(err);
		...
		it('will give subsidy', function(done) {
			...
			bscnd.getSubsidy(0, function(err, result) {
				should.not.exist(err);
				getSubsidy.callCount.should.equal(1);
	...
	describe('#getJsonBlock', function() {
		it('will give rpc error from getBlockHash', function(done) {
			...
			bscnd.getJsonBlock(0, function(err) {
				should.exist(err);
				bscnd._tryAllClients.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give rpc error from getBlock', function(done) {
			...
			bscnd.getJsonBlock('2f46b26835e9d768edd2afd393bdaf16cb141a95a663dc1c303e086a8af05d23', function(err) {
				should.exist(err);
				bscnd._tryAllClients.callCount.should.equal(1);
				getBlock.callCount.should.equal(1);
				err.should.be.an.instanceof(errors.RPCError);
		...
		it('will give jsonBlock using block height', function(done) {
			...
			bscnd.getJsonBlock(80000, function(err, result) {
				should.not.exist(err);
				bscnd._tryAllClients.callCount.should.equal(2);
				bscnd._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(1);
		...
		it('will give jsonBlock using blockhash', function(done) {
			...
			bscnd.getJsonBlock('2f46b26835e9d768edd2afd393bdaf16cb141a95a663dc1c303e086a8af05d23', function(err, result) {
				should.not.exist(err);
				bscnd._tryAllClients.callCount.should.equal(1);
				bscnd._maybeGetBlockHash.callCount.should.equal(1);
				getBlockHash.callCount.should.equal(0);
	...
	describe('#subscribeBalance', function() {
		...
		it('will not add an invalid address', function() {
			var bscnd = new BscnService(baseConfig);
			var emitter = new EventEmitter();
			bscnd.subscribeBalance(emitter, ['invalidaddress']);
			should.not.exist(bscnd.subscriptions.balance['invalidaddress']);
			log.info.callCount.should.equal(1);
		});
		it('will add a valid address', function() {
			var bscnd = new BscnService(baseConfig);
			var emitter = new EventEmitter();
			bscnd.subscribeBalance(emitter, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(bscnd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			log.info.callCount.should.equal(1);
		...
		it('will handle multiple address subscribers', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			bscnd.subscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			bscnd.subscribeBalance(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(bscnd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			bscnd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
		...
		it('will not add the same emitter twice', function() {
			...
			bscnd.subscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.exist(bscnd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			bscnd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			log.info.callCount.should.equal(2);
		});
	});
	...
	describe('#unsubscribeBalance', function() {
		...
		it('it will remove a subscription', function() {
			...
			should.exist(bscnd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			bscnd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			bscnd.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			bscnd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
		...
		it('will unsubscribe subscriptions for an emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			bscnd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			bscnd.unsubscribeBalance(emitter1);
			bscnd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			log.info.callCount.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing address', function() {
			...
			var emitter2 = new EventEmitter();
			bscnd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			bscnd.unsubscribeBalance(emitter1, ['qNq9mhTgH7KzKKDDwQ87Ain7mtyktheXyX']);
			bscnd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(2);
			log.info.callCount.should.equal(1);
		});
		it('will NOT unsubscribe subscription with missing emitter', function() {
			...
			var emitter1 = new EventEmitter();
			var emitter2 = new EventEmitter();
			bscnd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter2];
			bscnd.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			bscnd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			bscnd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'][0].should.equal(emitter2);
			log.info.callCount.should.equal(1);
		});
		it('will remove empty addresses', function() {
			var bscnd = new BscnService(baseConfig);
			...
			bscnd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			bscnd.unsubscribeBalance(emitter1, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			bscnd.unsubscribeBalance(emitter2, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			should.not.exist(bscnd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z']);
			log.info.callCount.should.equal(2);
		});
		it('will unsubscribe emitter for all addresses', function() {
			var bscnd = new BscnService(baseConfig);
			...
			var emitter2 = new EventEmitter();
			bscnd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			bscnd.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			sinon.spy(bscnd, 'unsubscribeBalanceAll');
			bscnd.unsubscribeBalance(emitter1);
			bscnd.unsubscribeBalanceAll.callCount.should.equal(1);
			bscnd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			bscnd.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			log.info.callCount.should.equal(1);
		});
	});
	...
	describe('#unsubscribeBalanceAll', function() {
		...
		it('will unsubscribe emitter for all addresses', function() {
			...
			bscnd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'] = [emitter1, emitter2];
			bscnd.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'] = [emitter1, emitter2];
			bscnd.subscriptions.balance['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'] = [emitter2];
			bscnd.subscriptions.balance['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR'] = [emitter1];
			bscnd.unsubscribeBalanceAll(emitter1);
			bscnd.subscriptions.balance['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'].length.should.equal(1);
			bscnd.subscriptions.balance['qRRv2uwzP5YSfWcnkcEUP25jvEDW7BJz1a'].length.should.equal(1);
			bscnd.subscriptions.balance['qeKn9hTqktwBRNGthi7YTfr8W7VKvZSgU2'].length.should.equal(1);
			should.not.exist(bscnd.subscriptions.balance['qS3MvbBY8y8xNZx2GVyMEdnQJTCPWoPLUR']);
			log.info.callCount.should.equal(1);
		});
	});
	...
	describe('#_notifyBalanceSubscribers', function() {
		...
		it('will notify balance subscribers', function(done) {
			...
			var bscnd = new BscnService(baseConfig);
			sinon.spy(bscnd, '_notifyBalanceSubscriber');
			bscnd._getAddressesFromTransaction = sinon.stub().returns(addresses);
			bscnd.getAddressSummary = sinon.stub().callsArgWith(2, null, {
			...
			emitter.on('quantumd/addressbalance', function(data) {
				...
				bscnd._notifyBalanceSubscriber.callCount.should.equal(1);
				bscnd._getAddressesFromTransaction.callCount.should.equal(1);
				emitter.emit.callCount.should.equal(1);
				done();
			});
			...
			bscnd._notifyBalanceSubscribers('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae', transaction);
	...
	describe('#_notifyBalanceSubscriber', function() {
		...
		it('will notify balance subscriber', function(done) {
			...
			emitter.on('quantumd/addressbalance', function(data) {
				...
				data.txid.should.equal('8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae');
				emitter.emit.callCount.should.equal(1);
				bscnd.getAddressSummary.callCount.should.equal(1);
				done();
			});
			...
			bscnd._notifyBalanceSubscriber('qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z', '8aad6af88ad09d8ce12c09586695c2abf8e5e38908f775e2d0c4b457ee0f4eae');
	...
	describe('#listUnspent', function() {
		it('will give rpc error', function(done) {
			...
			bscnd.listUnspent(0, 65000, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'], function(err) {
				should.exist(err);
		...
		it('will give array of unspent transaction outputs', function(done) {
			...
			bscnd.listUnspent(0, 65000, ['qfFYCmU7ACsDUbor4Do9AP1XxkocEKhs3z'], function(err, result) {
				should.not.exist(err);
				listUnspent.callCount.should.equal(1);
	...
	describe('#getNewAddress', function() {
		it('will give rpc error', function(done) {
			...
			bscnd.getNewAddress(function(err, result) {
				should.exist(err);
	...
	describe('#callContract', function() {
		it('will give rpc error', function(done) {
			...
			bscnd.callContract('f6177bc9812eeb531907621af6641a41133dea9e', 'd7bb99ba', {}, function(err, result) {
				should.exist(err);
	...
	describe('#getAccountInfo', function() {
		it('will give rpc error', function(done) {
			...
			bscnd.getAccountInfo('f6177bc9812eeb531907621af6641a41133dea9e', function(err, result) {
				should.exist(err);
	...
	describe('#getTransactionReceipt', function() {
		it('will give rpc error', function(done) {
			...
			bscnd.getTransactionReceipt('51b34cbcaf6fe3b687bf3d954f3baaeb377f5d0a3a8cdd29d899c049f6954a49', function(err, result) {
				should.exist(err);
				bscnd._tryAllClients.callCount.should.equal(1);
	...
	describe('#getDgpInfo', function() {
		it('will give rpc error', function(done) {
			...
			bscnd.getDgpInfo(function(err, result) {
				should.exist(err);
				bscnd._tryAllClients.callCount.should.equal(1);
	...
	describe('#getMiningInfo', function() {
		it('will give rpc error', function(done) {
			...
			bscnd.getMiningInfo(function(err, result) {
				should.exist(err);
				bscnd._tryAllClients.callCount.should.equal(1);
	...
	describe('#getStakingInfo', function() {
		it('will give rpc error', function(done) {
			...
			bscnd.getStakingInfo(function(err, result) {
				should.exist(err);
				bscnd._tryAllClients.callCount.should.equal(1);
